...
#
#Modified by
#2015 08 Becket Hui v0.0
#2015 09 Becket Hui v1.0
#2016 05 Becket Hui v2.0 add + between two consecutive organs/targets w/o operator,
#                        add support type library, roiChangeName function changed
####################################
...
def rename(inName):
 ...
 supportLib = stdNameList.supportList()
 ...
 ##check support names in the string##
 while True:
  for ii in range(len(sD)):
   if sD[ii].chk == False: #check each dissected string
    maxMatLen = 0
    for key in supportLib:
     for alt in supportLib.get(key): #check against each elements in the target lib
      sre = re.search(alt,sD[ii].s)
      if sre: #if there is a match
       if len(sre.group()) > maxMatLen:
        maxMatLen = len(sre.group()) #look for string that had max matched characters
        namToRepl = alt
     if maxMatLen > 0: #if there was match, split the current string based on the matched key
      sp_rp_StrDiss(sD, ii, namToRepl, key, 'Sup') #split the string list[ii]
      break #break the key for loop
    if maxMatLen > 0:
     break #if there was match, break the ii for loop
    else:
 ...
 ##combine organs and targets with + if no operators presented between them
 ##if first word is target, and second word is organ, keep underscore
 while True:
  for ii in range(len(sD)-1):
   sD[ii].chk = True
   if sD[ii].typ == 'Org' and (sD[ii+1].typ == 'Tar' or sD[ii+1].typ == 'Org'):
    sD.insert(ii+1,nameStr('+'))
    sD[ii+1].typ = 'Opr'
    sD[ii+1].std = True
    break #break for loop
  if ii >= len(sD)-2:
